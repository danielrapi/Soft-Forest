...
class SoftTreeEnsemble(torch.nn.Module):
    ...
    def __init__(self, num_trees, max_depth, leaf_dims, input_dim, activation='sigmoid', node_index=0,
                 internal_eps = 0, combine_output = True, subset_selection = False, subset_share = None):
        """
        ...
        if not self.leaf:
          ...
          self.left_child = SoftTreeEnsemble(
              ...
              combine_output = self.combine_output, subset_selection = self.subset_selection, subset_share = self.subset_share
              )
          ...
          self.right_child = SoftTreeEnsemble(
              ...
              subset_selection = self.subset_selection, subset_share = self.subset_share
              )
